...
const createStats = (config: any = {}) => {
  ...
  return {
    gauge(metric, value, tags = [], callback?) {
      return datadog.gauge(prefix + metric, value, appendHostName(tags), callback);
    },
    increment(metric, value, tags = [], callback?) {
      return datadog.increment(prefix + metric, value, appendHostName(tags), callback);
    },
    event(title, text, properties: any = {}, callback?) {
      properties.hostname = properties.host || properties.hostname || os.hostname();

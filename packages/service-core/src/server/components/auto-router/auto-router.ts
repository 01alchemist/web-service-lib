...
type RestApiMethods = Partial<{ [K in typeof methods[number]]: RequestHandler }>;
  ...
  return (RouteClass: IRestApiRoute) => {
    ...
    methods.forEach(method => {
      ...
      if (restApiMethod) {
        ...
        const routeKey = `${method}:${params.path}`;
        const existingRoute = routeMap.get(routeKey);
        if (existingRoute) {
          routeMap.set(routeKey, restApiMethod);
          logger.info(`ROUTE REPLACED ${params.path} => ${route.constructor.name}.${method}`);
        } else {
          logger.info(`ROUTE REGISTRED ${params.path} => ${route.constructor.name}.${method}`);
          ...
          router[method](
            ...
            async (req: express.Request, res: express.Response, next: express.NextFunction) => {
              ...
              if (requestHandler) {
                console.log("----> requestHandler.private:", requestHandler.private);
                if (requestHandler.private) {
                  const isSecurityCheckPassed = await doSecurityCheck(req, res);
                  console.log("----> isSecurityCheckPassed:", isSecurityCheckPassed);
                  if (!isSecurityCheckPassed) {
                    return;
                  }
                }
                try {
                  const receipt: RequestReceipt = await requestHandler(req, res, next);
                  if (receipt.status === RequestStatus.SUCCESS_WITH_WARNING) {
                    Sentry.captureMessage(receipt.message, Sentry.Severity.Warning);
                  }
                } catch (e) {
                  ...
                  res.status(error.code || 500).json({
                    status: false,
                    error: (error.response && error.response.error) || error.message || "Unknown error"
                  });
                }
              } else {
                res.header("Cache-Control", `max-age=0`);
              ...
              if (isDevelopmentEnv) {
                logger.info(`PATH -> [${protocol}] ${req.path} [${endTime - startTime}ms]`);

...
export const sha512 = (password: string, salt: string): { salt: string; passwordHash: string } => {
...
): { salt: string; passwordHash: string } => sha512(password, genRandomString(length));

export const verifyPassword = (password: string, salt: string, passwordHash: string): boolean =>
    sha512(password, salt).passwordHash === passwordHash;
